...
export default function RepresentativeData() {
  const { user } = useAuth();
  const { register, handleSubmit, formState: { errors }, reset } = useForm({
    resolver: zodResolver(schema),
    defaultValues: {
      ddi: '+55',
      ...JSON.parse(sessionStorage.getItem('representativeData') || '{}'),
    },
  });
    ...
    const loadSavedData = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          reset(data);
          sessionStorage.setItem('representativeData', JSON.stringify(data));
        }
      } catch (error) {
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      sessionStorage.setItem('currentStep', 'address');
